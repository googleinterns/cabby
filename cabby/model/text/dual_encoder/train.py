...
class Trainer:
  ...
  def evaluate(self):
    ...
    for batch in self.valid_loader:
      ...
      loss = self.compute_loss(text, attention_mask, cellids, neighbor_cells, far_cells)

      loss_val_total += loss.item()

      text_embedding, cellid_embedding = self.model(text, attention_mask, self.cells_tensor)
  ...
  def compute_loss(
    ...
    text_embedding, cellid_embedding = self.model(text, attention_mask,  cellids)
