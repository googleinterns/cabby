...
class Map:
    ...
    def __init__(self, map_name: Text, level: int, load_directory: Text = None):
        assert map_name == "Manhattan" or map_name == "Pittsburgh" or map_name == "Bologna"
        ...
        if map_name == "Manhattan":
            ...
            self.polygon_area = box(
                miny=44.4902, minx=11.3333, maxy=44.5000, maxx=11.3564, ccw=True)
    ...
    def write_map(self, dir_name: Text):
        ...
        if not os.path.exists(path):
            ...
            print ("path {0} already exists".format(path)) #TODO change to logger once it is merged
    ...
    def load_map(self, dir_name: Text):
        ...
        base_filename = self.map_name.lower()+"_poi"
        path = os.path.join(dir_name, base_filename + ".pkl")
        assert os.path.exists(path), "path {0} doesn't exists. Current directory {1}.".format(path, os.getcwd())
        ...
        base_filename = self.map_name.lower() + "_streets"
        path = os.path.join(dir_name, base_filename + ".pkl")
        assert os.path.exists(path), "path {0} doesn't exists.".format(path)
